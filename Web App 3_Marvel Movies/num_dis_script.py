...
chart_def = """
{
    ...
    yAxis: {
        title: {
            text: 'Number of Movies Distributed'
        },
        labels: {
            format: '{value}'
        },
...
def app():
    wp = jp.QuasarPage()
    h1 = jp.QDiv(a = wp, text = 'Analysis of Marvel Movie Distributors', classes = 'text-h3 text-center q-pa-md')

    hc = jp.HighCharts(a = wp, options = chart_def)
    hc.options.xAxis.categories = list(distributor.index)
    hc.options.series[0].data = list(distributor['Title'])
